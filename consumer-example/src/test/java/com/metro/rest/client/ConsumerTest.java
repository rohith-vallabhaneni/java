...
/**
 * It generates a pact file contains consumer contract which is suppose to be
...
public class ConsumerTest extends ConsumerPactTestMk2 {
	...
	@Rule
	public PactProviderRuleMk2 mockProvider = new PactProviderRuleMk2("platform_cdc_javaProvider", "localhost", 8080,
			this);
	...
	protected RequestResponsePact createPact(PactDslWithProvider builder) {
		return builder.given("test state").uponReceiving("ExampleJavaConsumerPactTest test interaction").path("/user")
				.method("GET").willRespondWith().status(200)
				.body(new PactDslJsonBody().integerType("id", 1).stringValue("name", "Rohith")).toPact();
	}
